...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_,address treasury_) {
        ...
        require(treasury_ != address(0), "ERC20: zero address.");
        treasury = treasury_;
        
        isExcludedTxFee[msg.sender] = true;
        isExcludedTxFee[address(this)] = true;
        _bufferBlock = block.number+20;
    }
	...
	function _transferFee(
        ...
        if(pancakeV2Pairs[from]){
             treasuryFee = amount.mul(fomoFee).div(_denominator);
            _takeFomoReward(from,to,amount,treasuryFee);
        }else if(pancakeV2Pairs[to]){
            treasuryFee = amount.mul(fundFee).div(_denominator);
            _takeFundReward(from,to,amount,treasuryFee);
        }
        _balances[to] = _balances[to].add(amount.sub(treasuryFee));
        emit Transfer(from, to, amount.sub(treasuryFee));         
	}
